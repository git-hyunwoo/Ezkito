...
def _convert_single_image_to_image(f, to_format: str) -> Tuple[io.BytesIO, str, str]:
    """Convert one image to another format (PNG/JPG/JPEG)."""
    img = _open_image_from_uploaded(f)

    # normalize
    to_format = (to_format or "").lower().strip()

    # Pillow expects "JPEG" (not "JPG")
    if to_format in ("jpg", "jpeg"):
        pil_format = "JPEG"
        out_ext = "jpg"  # keep extension consistent
        mime = "image/jpeg"

        # JPEG does not support alpha channel -> flatten if needed
        if img.mode in ("RGBA", "LA") or (img.mode == "P" and "transparency" in img.info):
            rgba = img.convert("RGBA")
            bg = Image.new("RGB", rgba.size, (255, 255, 255))  # white background
        ...
        # just in case (should be filtered by IMAGE_FORMATS)
        raise ValueError(f"Unsupported target image format: {to_format}")
    ...
    base = f.name.rsplit(".", 1)[0]
    filename = f"{base}_ezkito.{out_ext}"
